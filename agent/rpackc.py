...
class RPC:
	...
	def send_stream (self, target, data=None):
		self.socket.send(msgpack.packb({'type': 'stream', 'target': target, 'data': data}))

	def send_call (self, target, data=None):
		self.socket.send(msgpack.packb({'type': 'call', 'target': target, 'data': data}))
			...
			while self.alive:
				...
				if pkt.get(b'type') == b'call' and self.handler and pkt.get(b'target'):
					if getattr(self.handler, pkt.get(b'target').decode('utf-8', 'ignore')):
						getattr(self.handler, pkt.get(b'target').decode('utf-8', 'ignore'))(self, pkt.get(b'data', None))
				else:
