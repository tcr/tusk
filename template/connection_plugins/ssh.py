    ...
    def _communicate(self, p, stdin, indata, su=False, sudoable=False, prompt=None):
        ...
        while True:
            ...
            if p.stdout in rfd:
                ...
                stdout = intercept_progress(stdout)
            if p.stderr in rfd:
                dat = os.read(p.stderr.fileno(), 9000)
                stderr += dat
                if dat == '':
    ...
    try:
        ...
        for line in lines:
            ...
            try:
                ...
                pass
            if j.has_key('ansible_progress'):
                if j.has_key('stdout'):
                    sys.stdout.write(j['stdout'].encode('utf-8'))
                if j.has_key('stderr'):
                    sys.stderr.write(stringc(j['stderr'].encode('utf-8'), 'red'))
                continue

            preserve.append(line)

        stdout = ''.join(preserve)
