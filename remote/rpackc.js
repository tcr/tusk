...
function serialize (type, target, data) {
  return msgpack.pack({
    'type': type,
...
RPC.prototype.sendCall = function (target, data) {
  this.socket.send(serialize('call', target, data));
}

RPC.prototype.sendStream = function (target, data) {
  this.socket.send(serialize('stream', target, data));
}
